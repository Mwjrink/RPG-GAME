...
public class ShiftyDalton extends Creature {
	...
	public ShiftyDalton(float x, float y, int width, int height) {
		...
		animDown = new Animation((4), Assets.getNpc6Down());
		animUp = new Animation((4), Assets.getNpc6Up());
		animRight = new Animation((4), Assets.getNpc6Right());
		animLeft = new Animation((4), Assets.getNpc6Left());
	}
	...
	@Override
	String getText(){
		return "1 Hello, My name is Shifty";
...
//			System.out.println("Anim state 2 in the slimy, snitchy, sneaky and and shifty classes(move straight down then earpes?)");
	...
	public boolean goToSweatyPost(){
		...
		if(y < 23*Handler.getTilewidth() && gg){
			yMotion += 5;
			moveY();
			return true;
		} else if (x < 45*Handler.getTilewidth() && gg) {
			xMotion += 5;
			moveX();
			return true;
		}
	...
	public boolean enterSweatyPost(){
		if (x < 46*Handler.getTilewidth() && gg) {
			...
			moveY();
			return true;
		} else if (x > 44*Handler.getTilewidth()) {
            xMotion -= 5;
            moveX();
            return true;
        }
