...
public class SnitchyDalton extends Creature {
	...
	public SnitchyDalton(float x, float y, int width, int height) {
		...
		animDown = new Animation((4), Assets.getNpc5Down());
		animUp = new Animation((4), Assets.getNpc5Up());
		animRight = new Animation((4), Assets.getNpc5Right());
		animLeft = new Animation((4), Assets.getNpc5Left());
	}
	...
	@Override
	String getText(){
		return "1 Hello, My name is Snitchy";
	...
	public boolean goToSweatyPost(){
		...
		if(y < 24*Handler.getTilewidth()){
			...
			moveY();
			return true;
		} else if (x < 48*Handler.getTilewidth()) {
			xMotion += 5;
			moveX();
			return true;
		}
